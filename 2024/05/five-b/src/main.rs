...
fn main() {
    ...
    for i in 0..updates.len() {
        ...
        if !right_order {
            sum = sum + sort_and_get_middle(&mut updates[i], &rules);
        }
    }
...
fn sort_and_get_middle(update: &mut Vec<u32>, map: &HashMap<u32, Vec<u32>>) -> u32 {
    let l = update.len();
    for i in 0..l {
        for j in 0..(l - (i+1)) {
            if bigger_than(update[j], update[j+1], map) {
                let tmp = update[j];
                update[j] = update[j+1];
                update[j+1] = tmp;
            }
...
fn bigger_than(a: u32, b: u32, map: &HashMap<u32, Vec<u32>>) -> bool {
    match map.get(&b) {
        Some(v) => v.contains(&a),
        None => false
    }
